...
$(board).on('scoreUpdate', function (e, info) {
    ...
    $(scoreLabel).empty();
    $(scoreLabel).append(info.score + " điểm");
...
$(document).on("mousedown touchstart", "#canvas", function (evt) {
    if ($("img").is(':animated') == false) {
        ...
        if (evt.type == "mousedown") {
            ...
            if (DEBUG) {
                console.log(evt);
                console.log(parseInt(evt.touches[0].clientX), "- (", parseInt(evt.target.offsetLeft), "+", parseInt(evt.target.offsetParent.offsetLeft), ") = ", xCoord);
                console.log(parseInt(evt.target.offsetTop), "- (", parseInt(evt.target.offsetParent.offsetTop), "+", parseInt(evt.touches[0].clientY), ") = ", yCoord);

                $("#DEBUG").empty();
                $("#DEBUG").append(" " + xCoord + " " + yCoord);
            }
        }
