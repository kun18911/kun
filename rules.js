var Rules = function(board)
{
  ...
  this.activateColorBomb = function(colorBomb) {
    var candiesToRemove = [];
    for (var row = 0; row < board.boardSize; row++) {
        for (var col = 0; col < board.boardSize; col++) {
            var candy = board.getCandyAt(row, col);
            if (candy && candy.color === colorBomb.color) {
    ...
    if (candiesToRemove.length > 0) {
        this.removeCrushes([candiesToRemove]);
        colorBomb.isColorBomb = false;
    }
}
  ...
  function updateScoreLabels() {
    ...
    document.getElementById('yellowPoints').textContent = yellowScore;
    document.getElementById('bluePoints').textContent = blueScore;
    document.getElementById('greenPoints').textContent = greenScore;
    document.getElementById('orangePoints').textContent = orangeScore;
    document.getElementById('purplePoints').textContent = purpleScore;
  }
  ...
  this.getRandomValidMove = function()
  {
    ...
    if (validMovesColorBomb.length > 0) {
      var colorBombMove = validMovesColorBomb[Math.floor(Math.random() * validMovesColorBomb.length)];
      this.activateColorBomb(colorBombMove.candy);
      return colorBombMove;
    }
